...
async function migratePhotos() {
  console.log('Starting photo migration to /albums/{userId}/ structure...')
  ...
  try {
    ...
    const photos = await prisma.userPhoto.findMany({
      where: {
        url: {
          contains: 'blob.vercel-storage.com/albums/'
        }
      },
      include: {
        user: true
    ...
    for (const photo of photos) {
      try {
        console.log(`Migrating photo ${photo.id} for user ${photo.userId}...`)
        ...
        // Generate new filename with userId
        const urlParts = photo.url.split('/')
        const oldFilename = urlParts[urlParts.length - 1]
        const newFilename = `albums/${photo.userId}/${oldFilename}`
