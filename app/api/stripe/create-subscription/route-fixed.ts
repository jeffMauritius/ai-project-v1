  ...
  try {
    ...
    try {
      ...
      if (existingPrice) {
        ...
        console.log('[SUBSCRIPTION] Prix existant trouvé:', stripePriceId)
      } else {
        // Créer le produit
        ...
        console.log('[SUBSCRIPTION] Nouveau produit et prix créés:', product.id, price.id)
      }
    } catch (stripeError) {
    ...
    return NextResponse.json(
      { error: 'Erreur lors de la création de l\'abonnement', details: error.message },
      { status: 500 }
    )
  }
}
